...
export function cardCanvas(card?: Card): HTMLCanvasElement {
    ...
    if (card) {
        ...
        c.fillStyle = suitColors[card.suit];
        ...
        if (gold || card.value === 2) {
            c.fillStyle = gold ? '#99702c' : suitColors[card.suit]; c.font = 'bold 21px Arial';
        ...
        c.fillStyle = '#b6b07e'; for (let i = 0; i < 7; i++) { c.beginPath(); c.arc((i % 2 ? 15 : -15), -95 + i * 20, 4, 0, Math.PI * 2); c.fill(); }
        c.font = '900 49px Georgia'; c.lineWidth = 3; c.strokeStyle = '#203e30'; c.fillStyle = '#e6dac1';
